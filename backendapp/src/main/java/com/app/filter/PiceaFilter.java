...
@WebFilter(filterName = "piceaFilter", urlPatterns = "/*" , initParams = {
        @WebInitParam(name = "URL", value = "http://localhost:8763")})
public class PiceaFilter implements Filter {
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        this.request.setAttribute("code", "success");
        this.request.setAttribute("msg", "注册成功，请马上到邮箱激活");
        //this.request.getRequestDispatcher("/query/ping").forward(request, response);
        this.response = ((HttpServletResponse) servletResponse);
        String url = request.getRequestURI();
        this.response.setHeader("source","test");
